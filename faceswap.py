...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "-i1",
        type=str,
        help='Source input folder filename, expects images ofform "%%04d.jpg". (Replaces faces of this sequence)',
    )
    parser.add_argument(
        "--target_folder",
        "-i2",
        type=str,
        help='Source input folder filename, expects images ofform "%%04d.jpg". (Faces are extracted of this sequence)',
    )
    parser.add_argument("--output_folder", "-o", type=str, default="output/images", help="Output image folder.")
    parser.add_argument("--output_mask_folder", "-mo", type=str, default="output/masks", help="Output mask folder.")
    parser.add_argument(
        "--max_frames",
        type=int,
        default=9000,
        help="Maximum number of frames, will not extract any more frames after reaching this number. "
        "9000 is  chosen simply because of the fact that there is no video in our dataset with longer length.",
    )
    ...
    for folder in [output_folder, output_mask_folder]:
        Path(folder).mkdir(exits_ok=True)
    ...
    while True:
        ...
        source_shapes2D = utils.getFaceKeypoints(source_img, detector, predictor, maxImageSizeForDetection)
        target_shapes2D = utils.getFaceKeypoints(target_img, detector, predictor, maxImageSizeForDetection)
        ...
        if (source_shapes2D is not None) and (target_shapes2D is not None):
            if len(source_shapes2D) == 1:
                ...
                source_shape2D_index = [np.max(shape2D[0]) - np.min(shape2D[0]) for shape2D in source_shapes2D]
                source_shape2D_index = np.argmax(source_shape2D_index)
                target_shape2D_index = [np.max(shape2D[0]) - np.min(shape2D[0]) for shape2D in target_shapes2D]
                target_shape2D_index = np.argmax(target_shape2D_index)
            else:
                source_shape2D_index = np.argmin(
                    [
                        np.min(np.abs(shape2D[0] - source_pointX) + np.abs(shape2D[1] - source_pointY))
                        for shape2D in source_shapes2D
                    ]
                )
                target_shape2D_index = np.argmin(
                    [
                        np.min(np.abs(shape2D[0] - target_pointX) + np.abs(shape2D[1] - target_pointY))
            ...
            textureCoords = utils.getShapeTextureCoords(source_shape2D, mean3DShape, blendshapes, idxs2D, idxs3D)
            renderer = FaceRendering.FaceRenderer(target_img, source_img, textureCoords, mesh)
            ...
            renderedImg = ImageProcessing.colorTransfer(target_img, renderedImg, mask)
            swapped_img = ImageProcessing.blendImages(renderedImg, swapped_img, mask)
        # Save files
