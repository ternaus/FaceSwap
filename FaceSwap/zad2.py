...
predictor = FaceAlignment(112, 112, 1, 1)
predictor.loadNetwork(predictor_path)
...
while True:
    cameraImg = cap.read()[1]
    shapes2D = utils.getFaceKeypoints(cameraImg, detector, predictor, prevShape2D)

    if shapes2D is not None:
        prevShape2D = shapes2D[0].T
        shape2D = shapes2D[0]
        ...
        if drawOverlay:
            ...
            drawProjectedShape(
                cameraImg, [mean3DShape, blendshapes], projectionModel, mesh, modelParams, lockedTranslation
            )
    ...
    if key == ord("r"):
        if writer is None:
            ...
            writer = cv2.VideoWriter(
                "../out.avi", cv2.cv.CV_FOURCC("X", "V", "I", "D"), 25, (cameraImg.shape[1], cameraImg.shape[0])
            )
