...
from FaceSwap import utils
from FaceSwap.AffineTransformLayer import AffineTransformLayer
from FaceSwap.LandmarkImageLayer import LandmarkImageLayer
from FaceSwap.LandmarkInitLayer import LandmarkInitLayer
from FaceSwap.LandmarkTranformLayer import LandmarkTransformLayer
from FaceSwap.TransformParamsLayer import TransformParamsLayer
...
class FaceAlignment:
    ...
    def addDANStage(self, stageIdx, net):
        ...
        net[prevStage + "_landmarks_affine"] = LandmarkTransformLayer(
            net[prevStage + "_landmarks"], net[prevStage + "_transform_params"]
        )
        net[prevStage + "_img_landmarks"] = LandmarkImageLayer(
            net[prevStage + "_landmarks_affine"], (self.imageHeight, self.imageWidth), self.landmarkPatchSize
        )

        net[prevStage + "_img_feature"] = lasagne.layers.DenseLayer(
            net[prevStage + "_fc1"], num_units=56 * 56, W=GlorotUniform("relu")
        )
        ...
        net[curStage + "_conv1_1"] = batch_norm(
            Conv2DLayer(net[curStage + "_input"], 64, 3, pad="same", W=GlorotUniform("relu"))
        )
        net[curStage + "_conv1_2"] = batch_norm(
            Conv2DLayer(net[curStage + "_conv1_1"], 64, 3, pad="same", W=GlorotUniform("relu"))
        )
        net[curStage + "_pool1"] = lasagne.layers.Pool2DLayer(net[curStage + "_conv1_2"], 2)

        net[curStage + "_conv2_1"] = batch_norm(
            Conv2DLayer(net[curStage + "_pool1"], 128, 3, pad=1, W=GlorotUniform("relu"))
        )
        net[curStage + "_conv2_2"] = batch_norm(
            Conv2DLayer(net[curStage + "_conv2_1"], 128, 3, pad=1, W=GlorotUniform("relu"))
        )
        net[curStage + "_pool2"] = lasagne.layers.Pool2DLayer(net[curStage + "_conv2_2"], 2)

        net[curStage + "_conv3_1"] = batch_norm(
            Conv2DLayer(net[curStage + "_pool2"], 256, 3, pad=1, W=GlorotUniform("relu"))
        )
        net[curStage + "_conv3_2"] = batch_norm(
            Conv2DLayer(net[curStage + "_conv3_1"], 256, 3, pad=1, W=GlorotUniform("relu"))
        )
        net[curStage + "_pool3"] = lasagne.layers.Pool2DLayer(net[curStage + "_conv3_2"], 2)

        net[curStage + "_conv4_1"] = batch_norm(
            Conv2DLayer(net[curStage + "_pool3"], 512, 3, pad=1, W=GlorotUniform("relu"))
        )
        net[curStage + "_conv4_2"] = batch_norm(
            Conv2DLayer(net[curStage + "_conv4_1"], 512, 3, pad=1, W=GlorotUniform("relu"))
        )
        ...
        net[curStage + "_fc1"] = batch_norm(
            lasagne.layers.DenseLayer(net[curStage + "_fc1_dropout"], num_units=256, W=GlorotUniform("relu"))
        )
        ...
        net[curStage + "_landmarks"] = LandmarkTransformLayer(
            net[curStage + "_landmarks"], net[prevStage + "_transform_params"], True
        )

    def createCNN(self):
        net = {}
        net["input"] = lasagne.layers.InputLayer(
            shape=(None, self.nChannels, self.imageHeight, self.imageWidth), input_var=self.data
        )
        print("Input shape: {}".format(net["input"].output_shape))
        ...
        net["s1_fc1"] = batch_norm(
            lasagne.layers.DenseLayer(net["s1_fc1_dropout"], num_units=256, W=GlorotUniform("relu"))
        )
    ...
    def CropResizeRotate(self, img, inputShape):
        ...
        for i in range(img.shape[0]):
            outImg[i] = ndimage.interpolation.affine_transform(
                img[i], A2, t2[[1, 0]], output_shape=(self.imageHeight, self.imageWidth)
            )
